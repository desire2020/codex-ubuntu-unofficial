...
function patchOfficialAsar(source, destination) {
  ...
  const contents = fs.readFileSync(mainBundle, "utf8");
  if (!contents.includes(oldSnippet)) {
    fail("Could not find the expected Vite/dev-mode branch in the official main bundle.");
  }
  fs.writeFileSync(mainBundle, contents.replace(oldSnippet, newSnippet));
  ...
  if (!bootstrapContents.includes(bootstrapOldSnippet)) {
    fail("Could not find the expected bootstrap error handler in the official bundle.");
  }
  fs.writeFileSync(bootstrapBundle, bootstrapContents.replace(bootstrapOldSnippet, bootstrapNewSnippet));
